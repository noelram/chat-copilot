...
export const PlanViewer: React.FC<PlanViewerProps> = ({
  ...
  const planState =
    parsedContent.state === PlanState.Derived
      ? PlanState.Derived
      : message.planState ?? parsedContent.state;
  const [plan, setPlan] = useState(originalPlan);

  const onPlanAction = async (
    planState: PlanState.Approved | PlanState.Rejected
  ) => {
  ...
  return (
    <div className={classes.container}>
      <Text>
        Based on the request, Chat Copilot will run the following steps:
      </Text>
      <PlanBody
        plan={plan}
        setPlan={setPlan}
