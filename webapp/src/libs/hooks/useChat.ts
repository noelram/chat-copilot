...
export const useChat = () => {
  ...
  const getResponse = async ({
    ...
    try {
      ...
      const responseTokenUsage = askResult.variables.find(
        (v) => v.key === "tokenUsage"
      )?.value;
      if (responseTokenUsage)
        dispatch(
          updateTokenUsage(JSON.parse(responseTokenUsage) as TokenUsage)
        );
    } catch (e: any) {
      ...
      const errorMessage = `Unable to ${action}. Details: ${getErrorDetails(
        e
      )}`;
      dispatch(addAlert({ message: errorMessage, type: AlertType.Error }));
    }
  ...
  const downloadBot = async (chatId: string) => {
    try {
      return await botService.downloadAsync(
        chatId,
        await AuthHelper.getSKaaSAccessToken(instance, inProgress)
      );
    } catch (e: any) {
      const errorMessage = `Unable to download the bot. Details: ${getErrorDetails(
        e
      )}`;
      dispatch(addAlert({ message: errorMessage, type: AlertType.Error }));
    }
  ...
  const uploadBot = async (bot: ChatArchive) => {
    try {
      ...
      const errorMessage = `Unable to upload the bot. Details: ${getErrorDetails(
        e
      )}`;
      dispatch(addAlert({ message: errorMessage, type: AlertType.Error }));
    }
  ...
  const importDocument = async (
    ...
    try {
      ...
      const contentSafetyDisabledRegEx =
        /Access denied: \[Content Safety] Failed to analyze image./g;
  ...
  const joinChat = async (chatId: string) => {
    try {
      ...
      await chatService
        .joinChatAsync(chatId, accessToken)
  ...
  const getServiceInfo = async () => {
    try {
      ...
      const errorMessage = `Error getting service options. Details: ${getErrorDetails(
        e
      )}`;
      dispatch(addAlert({ message: errorMessage, type: AlertType.Error }));
